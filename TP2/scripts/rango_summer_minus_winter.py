...
def mapa(dato,titulo):
    ...
    levels = [dato.min(),3,dato.max()]
    ax1.contourf(cyclic_lons, lat, dato,levels, transform=data_crs,levels=levels, hatches=["", "..."], alpha=0)
...
titulo = 'Dominio monzón HS CanESM2'
...
plt.savefig(path+'dominio_CanESM2_HS.png',bbox_inches='tight')
plt.clf

titulo = 'Dominio monzon HN CanESM2'
lon = np.arange(0, 362, 362/144)
mon_range_CanESM2 = annual_range_HN(CanESM2_ens)
lat = mon_range_CanESM2.lat
cyclic_data, cyclic_lons = add_cyclic_point(mon_range_CanESM2, coord=lon)
mapa(cyclic_data,titulo)
plt.savefig(path+'dominio_CanESM2_HN.png',bbox_inches='tight')
plt.clf
...
mapa(cyclic_data,titulo)
plt.savefig(path+'DJF_JJA_range_CanESM5_HS.png',bbox_inches='tight')
plt.clf
...
cyclic_data, cyclic_lons = add_cyclic_point(mon_range_CanESM5, coord=lon)
mapa(cyclic_data,titulo)
plt.savefig(path+'JJA_DJF_CanESM5_HN.png',bbox_inches='tight')
plt.clf
