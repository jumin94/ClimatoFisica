...
def bandas_lat(dato,indice):
    ...
    area = sum(weights_aux)
    aux_weightHN = auxHN.sum(dim='lat')/area
    aux_weightHS = auxHS.sum(dim='lat')/area
...
C = pd.DataFrame(columns=['90-80N','80-70N','70-60N','60-50N','50-40N','40-30N','30-20N','20-10N','10-0N','0-10S','20-10S','30-20S','40-30S','50-40S','60-50S','70-60S','80-70S','90-80S','90-0N','90-0S'])
...
Precip_hist = pd.concat([C,bandas_lat(pr_ens_hist,'ens_hist')],axis=0)
for i in range(len(pr_hist)):
    Precip_hist = pd.concat([Precip_hist,bandas_lat(pr_hist[i],'hist_r'+str(i+1))],axis=0)

Precip_rcp26 = pd.concat([C,bandas_lat(pr_ens_rcp26,'ens_rcp26')],axis=0)
for i in range(len(pr_rcp26)):
    Precip_rcp26 = pd.concat([Precip_rcp26,bandas_lat(pr_rcp26[i],'rcp26_r'+str(i+1))],axis=0)

Precip_rcp85 = pd.concat([C,bandas_lat(pr_ens_rcp85,'ens_rcp85')],axis=0)
for i in range(len(pr_rcp85)):
    Precip_rcp85 = pd.concat([Precip_rcp85,bandas_lat(pr_rcp85[i],'rcp85_r'+str(i+1))],axis=0)
