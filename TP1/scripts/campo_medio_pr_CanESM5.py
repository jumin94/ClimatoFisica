...
list = []
list = glob.glob('pr*historical*p2f1_2.5_anu.nc')

#Guardo las temperaturas en temp_obs
temp_obs =  xr.open_dataset(path+'DATOS/precip.mon.total.v7_197601-200512_2.5_anu.nc')
...
for run in tas_mod:
    clima, std = modaly(run,temp_obs.precip)
    sd.append(std)

clima, std_ens = modaly(tas_mod_ens,temp_obs.precip)
...
def mapa(dato,titulo):
    ...
    clevels = np.arange(-2000,2000,200)
    ...
    cbar.set_label('Precipitacion [mm/año]',fontsize=20)
    return fig

titulo = 'bias en pr CanESM5'
lon = np.linspace(0,360,144)
mapa(climatologia.precip-tas_ens,titulo)
plt.savefig(path+'climatologia_bias_CanESM5_pr.png',bbox_inches='tight')
plt.clf
titulo = 'standard deviation ensamble CanESM5'
mapa(std_ens,titulo)
plt.savefig(path+'desviacion_standard_ensamble_CanESM5_pr.png',bbox_inches='tight')
plt.clf
